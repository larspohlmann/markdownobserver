...
struct ReaderCSSFactory {
    ...
    func makeCSS(theme: ThemeDefinition, syntaxTheme: SyntaxThemeKind, baseFontSize: Double) -> String {
        ...
        let syntaxLayer = theme.providesSyntaxHighlighting ? (theme.syntaxCSS ?? "") : syntaxTheme.css
    ...
    func makeHTMLDocument(
        ...
        <body>
          ...
          \(bootstrapRuntime)
          \(themeJavaScript.map { "<script>\($0)</script>" } ?? "")
        </body>
        </html>
        """
    }
    ...
    private func makeBootstrapRuntime(payloadBase64: String, cssBase64: String) -> String {
        ...
        let inlineDiffJS = Self.loadBundledJS(named: "markdownobserver-inline-diff")
        let runtimeJS = Self.loadBundledJS(named: "markdownobserver-runtime")
